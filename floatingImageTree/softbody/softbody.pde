...
//DESTINATION
    ...
    //move nodes
    for (int i=0; i<nodes; i++){
      float moveValueX = map(radius, RADIUS_MIN, RADIUS_MAX, 0.5, 2);
      float moveValueY = map(radius, RADIUS_MIN, RADIUS_MAX, 0.1, 1);
      nodeX[i] = nodeStartX[i]+sin(radians(angle[i]))*(accelX*moveValueX);
      nodeY[i] = nodeStartY[i]+sin(radians(angle[i]))*(accelY*moveValueY);
      angle[i]+=frequency[i];
    }
  }
  void update(){
