...
class SoftBody{
  ...
  void drawShape() {
    ...
    float sizeChangeValue = (startDistanceFromDestination-distanceFromDestination)/startDistanceFromDestination;
    for (int i=0; i<nodes; i++){
      nodeStartX[i] = centerX+cos(radians(rotAngle))*radius*pow(sizeChangeValue, 0.5);
      nodeStartY[i] = centerY+sin(radians(rotAngle))*radius*pow(sizeChangeValue, 0.5);
...
//DESTINATION
    ...
    //move nodes
    for (int i=0; i<nodes; i++){
      nodeX[i] = nodeStartX[i]+sin(radians(angle[i]))*(accelX*1.2);
      nodeY[i] = nodeStartY[i]+sin(radians(angle[i]))*(accelY*1.2);
      angle[i]+=frequency[i];
    }
  }
  void update(){
